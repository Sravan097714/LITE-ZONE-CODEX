report 50019 "Sales Order"
{
    ...
    dataset
    {
        dataitem("Sales Header"; "Sales Header")
        {
            ...
            column(gboolHideColumn; gboolHideColumn) { }
            dataitem(CopyLoop; Integer)
            {
                ...
                dataitem(PageLoop; Integer)
                {
                    ...
                    column(CompanyEmail2; CompanyInfo."Order Confirmation Email") { }
                    ...
                    column(Dept; Dept) { }
                    column(UPPERCASE_CompanyInfo_Name; CompanyInfo.Name) { }
                    ...
                    dataitem("Sales Line"; "Sales Line")
                    {
                        ...
                        column(Quantity_SalesInvoiceLine; "Sales Line"."Qty. to Ship") { }
                        column(LineAmount2_SalesInvoiceLine; gdecAmountInclVAT) { }
                        column(LineAmount_SalesInvoiceLine; gdecAmountExclVAT) { }
                        column(AmountExcVAT; "Sales Line"."Amount Including VAT" - Amount) { }
                        column(VATStatus; VATStatus) { }
                        column(AmountIncludingVAT_SalesInvoiceLine; gdecAmountInclVAT) { }
                        column(LineAmt; LineAmt) { }
                        column(Amount_Including_VAT_Amount; gdecAmountInclVAT - ("Sales Line"."Qty. to Ship" * "Sales Line"."Unit Price")) { }
                        column(VATPerc; VATPerc) { }
                        column(VatAmount; VatAmount) { }
                        column(AmtIncl; AmtIncl) { }
                        column(AmountIncludingVAT; gdecAmountInclVAT) { }
                        ...
                        begin
                            ...
                            Clear(gdecAmountExclVAT);
                            gdecAmountExclVAT := "Sales Line"."Qty. to Ship" * "Sales Line"."Unit Price";

                            Clear(gdecAmountInclVAT);
                            clear(gdecVATAmount);
                            if "VAT %" <> 0 then
                                gdecVATAmount := "VAT %" / 100 * gdecAmountExclVAT;

                            Clear(gdecAmountInclVAT);
                            gdecAmountInclVAT := gdecAmountExclVAT + gdecVATAmount;
    ...
    trigger OnPreReport()
    begin
        gboolHideColumn := false;
        if grecUserSetup.Get(UserId) then
            gboolHideColumn := grecUserSetup."Hide field on Ord Confirmation";
    end;

    var
        gdecAmountExclVAT: Decimal;
        gdecVATAmount: Decimal;
        gdecAmountInclVAT: Decimal;
        GLSetup: Record 98;
